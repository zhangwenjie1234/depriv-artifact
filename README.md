...
├── tests/                          # Unit tests for risk, control, and AMC utilities
├── observation1.py                 # Directional utility sensitivity plot
├── observation2.py                 # Party-level label leakage plot
├── run_forward_norm_observation.py # Party-level representation scales
├── run_amc_attack.py               # AMC comparison with defense baselines
├── run_risk_threshold_amc_sensitivity.py
├── run_budget_split_sensitivity.py
├── run_clip_quantile_sensitivity.py
├── run_rl_control_convergence.py
├── run_adaptive_fixed_all.py
├── run_full_vs_fixed_control.py     # Paired driver used by the previous script
├── run_runtime_overhead.py
...
To run the optional test suite, install `pytest` and execute:
...
The observation scripts read metric JSON files and can export PDF figures.
For example:

```bash
python observation1.py \
  --dataset mnist --mode forward_only \
  --output figures/observation1_forward.pdf

python observation2.py \
  --dataset mnist --num_passive 4 --attack cluster --use_emb 1 \
  --output figures/observation2.pdf

python -u run_forward_norm_observation.py \
  --gpu 0 --datasets mnist fashionmnist cifar10
```

The threshold and training-dynamics drivers also provide `--plot-only` modes
for regenerating figures without rerunning training.
